...
let
  ...
  rgbaApps = {
    ...
    fileManagers = "(?i)^thunar$|^org\\.xfce\\.Thunar$|^org\\.gnome\\.FileRoller$|^org\\.kde\\.dolphin$|^dolphin$|^org\\.gnome\\.Nautilus$|^nautilus$|^nemo$|^org\\.nemo\\.Nemo$|^pcmanfm$|^pcmanfm-qt$|^org\\.lxde\\.PCManFM$|^org\\.lxqt\\.pcmanfm-qt$|^org\\.kde\\.krusader$|^krusader$";
    ...
    editors = "^codex-desktop$|^ChatGPT$|^code$|^code-url-handler$|^com\\.visualstudio\\.code$|^code-oss$|^codium$|^vscodium$|^cursor$|^zed$|^dev\\.zed\\.Zed$|^t3-code$|^T3 Code.*$|^windsurf$|^jetbrains-.*$|^android-studio$|^neovide$|^emacs$|^micro$|^orca$";
